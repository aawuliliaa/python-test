...
class FtpServer(object):
    ...
    def get_response_from_client(self, request_conn_obj):
        """
        接收客户端发送的数据
        :return:
        """
        # 客户端发送请求命令时，把数据封装为1024字节，避免粘包问题
        client_request_cmd = request_conn_obj.recv(self.MSG_SIZE)
    ...
    def handle_client_request(self, request_conn_obj):
        """
        接收用户请求，并处理
        :return:
        """
        while True:
            client_request_cmd = self.get_response_from_client(request_conn_obj)
    ...
    def send_certain_size_response(self, response_code, request_conn_obj, **kwargs):
        ...
        request_conn_obj.send(bytes_response_data)
    ...
    def del_unfinished_put(self, client_request_cmd, request_conn_obj):
        """
        ...
        for file in file_put_unfinished_list:
            os.remove(file)
        self.send_certain_size_response("204", request_conn_obj)

    def re_put(self, client_request_cmd, request_conn_obj):
        """
        断点重新上传
        :param client_request_cmd:
        :param request_conn_obj:
        ...
        self.send_certain_size_response("203", request_conn_obj, received_file_size=received_file_size)
        f = open(file_server_abs_path, "ab")
        self.receive_data_from_client(f, received_file_size, file_total_size, request_conn_obj)
        # 验证md5
        self.verify_md5(source_file_md5_value, file_server_abs_path)

    def receive_data_from_client(self, f_obj, received_file_size, file_total_size, request_conn_obj):
        """
        从客户端接收数据，写入服务端的文件中
        :param f_obj:
        ...
        while received_file_size < file_total_size:
            ...
            if left_file_size < self.MSG_SIZE:
                data = request_conn_obj.recv(left_file_size)
                # 这样操作是不可以的len(data)!=left_file_size
                # received_file_size += left_file_size
            else:
                data = request_conn_obj.recv(self.MSG_SIZE)
        ...
        f_obj.close()
        self.user_info[current_thread_name]["left_quota"] -= file_total_size
        username = self.user_info[current_thread_name]["username"]

        # self.left_quota -= file_total_size
        self.accounts[username]["left_quota"] = str(self.user_info[current_thread_name]["left_quota"])
        self.accounts.write((open(settings.ACCOUNT_FILE, "w")))

    def put(self, client_request_cmd, request_conn_obj):
        """
        处理文件上传,包含md5验证
        :param client_request_cmd:
        :param request_conn_obj:
        ...
        file_abs_path = os.path.join(user_current_dir, file_name)
        if os.path.exists(file_abs_path):
            file_name = "%s-%s" % (uuid.uuid1(), file_name)
            file_abs_path = os.path.join(user_current_dir, file_name)
        self.send_certain_size_response("202", request_conn_obj, file_name=file_name, file_abs_path=file_abs_path)
    ...
    def ls(self, client_request_cmd, request_conn_obj):
        """
        列出当前目录下的内容
        :param client_request_cmd:
        :param request_conn_obj:
        ...
        if client_request_cmd:
            cmd_obj = subprocess.Popen("dir %s" % user_current_dir, shell=True,
            ...
            self.send_certain_size_response("300", request_conn_obj, cmd_result_total_size=cmd_result_total_size)
            request_conn_obj.sendall(cmd_result)
